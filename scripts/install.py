#!/usr/bin/env python3
"""Install pinned Deliver Product skill packages with drift detection."""
...
DEFAULT_SOURCE = "https://github.com/omiologic/deliver-product.git"
DEFAULT_REF = "main"
MANIFEST_NAME = ".deliver-product-install.json"
PACKAGES = (
    "deliver-product",
    "delivery-execution",
    "delivery-planning",
...
def fetch_snapshot(
    ...
    for command in commands:
        ...
        if result.returncode != 0:
            ...
            raise InstallError(f"cannot fetch Deliver Product: {detail}")
    revision = runner(
...
def main(argv: list[str] | None = None, input_fn: Callable[[str], str] = input) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="deliver-product-install-") as temp:
        workspace = Path(temp)
