...
Target, WorkItem, preflight, and archive-gate operations must not return unrelated registrations, claims, baselines, or history. Validation may scan every record in deterministic code, but it emits at most 100 ordered diagnostics plus an omitted count so the full corpus is not injected into model context.
