...
The skills own thin default guardrails and use the consumer's conventions and contracts for planning preferences, terminology, and local adapters. Advisory Delivery requires no `_notes/DELIVERY.md` or `_notes/plans/**` tree. When the repository-local planning adapter is selected, target retrieval loads only the selected WorkItem, its dependency closure, and applicable archived summaries; lifecycle scans stay metadata-only, while audit and validation remain explicit. When persistence is explicitly requested, configured paths are resolved relative to the consumer, and repository-local records remain projections rather than canonical runtime state. Delivery Spine supports bounded schema-v2 sharded retrieval while retaining a schema-v1 monolithic compatibility reader. Its selected records may use versioned, benchmark-gated ephemeral agent views while compact JSON remains the default and fallback.
