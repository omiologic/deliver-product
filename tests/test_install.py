...
SPEC = importlib.util.spec_from_file_location("deliver_product_install", ROOT / "scripts" / "install.py")
...
class InstallerTests(unittest.TestCase):
    ...
    def snapshot(self, root: Path, marker: str = "first", commit: str = "a" * 40):
        ...
        return INSTALL.Snapshot(package_root, "https://example.invalid/deliver-product.git", commit, digests)
    ...
    def test_selected_subset_installs_in_one_invocation(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            target.mkdir()
            snapshot = self.snapshot(root)
            selected = ("deliver-product", "delivery-planning")
            states, manifest = INSTALL.inspect_installation(self.skill_root(target), snapshot, selected)
            INSTALL.apply_installation(self.skill_root(target), snapshot, states, manifest)
            self.assertTrue((self.skill_root(target) / "deliver-product").is_dir())
            self.assertTrue((self.skill_root(target) / "delivery-planning").is_dir())
            self.assertFalse((self.skill_root(target) / "delivery-execution").exists())
    ...
    def test_upstream_ahead_is_reported_without_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            original = (self.skill_root(target) / "deliver-product" / "SKILL.md").read_bytes()
            ...
            self.assertEqual(original, (self.skill_root(target) / "deliver-product" / "SKILL.md").read_bytes())
            ...
            self.assertEqual(
                (second.root / "deliver-product" / "SKILL.md").read_bytes(),
                (self.skill_root(target) / "deliver-product" / "SKILL.md").read_bytes(),
            )
    ...
    def test_matching_migration_records_manifest_without_recopy(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            INSTALL._copy_package(snapshot.root / "delivery-planning", skill_root / "delivery-planning", "delivery-planning")
            package_file = skill_root / "delivery-planning" / "SKILL.md"
            before = package_file.stat().st_mtime_ns
            states, manifest = INSTALL.inspect_installation(skill_root, snapshot, ("delivery-planning",))
    ...
    def test_differing_migration_lists_paths_and_requires_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            skill_root.mkdir(parents=True)
            INSTALL._copy_package(snapshot.root / "delivery-planning", skill_root / "delivery-planning", "delivery-planning")
            changed = skill_root / "delivery-planning" / "references" / "contract.md"
            ...
            with patch.object(INSTALL, "fetch_snapshot", return_value=snapshot), redirect_stdout(output):
                result = INSTALL.main(
                    ["--target", str(target), "--agent", "codex", "--package", "delivery-planning", "--migrate"],
            ...
            with patch.object(INSTALL, "fetch_snapshot", return_value=snapshot):
                result = INSTALL.main(
                    ["--target", str(target), "--agent", "codex", "--package", "delivery-planning", "--migrate", "--yes"]
                )
            self.assertEqual(0, result)
            self.assertEqual(
                (snapshot.root / "delivery-planning" / "references" / "contract.md").read_text(encoding="utf-8"),
    ...
    def test_local_modification_is_rejected_with_path_and_no_changes(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            snapshot = self.snapshot(root)
            skill_root = self.skill_root(target)
            states, manifest = INSTALL.inspect_installation(skill_root, snapshot, ("delivery-execution",))
            INSTALL.apply_installation(skill_root, snapshot, states, manifest)
            changed = skill_root / "delivery-execution" / "SKILL.md"
            changed.write_text("local modification", encoding="utf-8")
            states, manifest = INSTALL.inspect_installation(
                skill_root, snapshot, ("delivery-execution",), baseline=snapshot
            )
