...
Repository-local planning retrieval is target-scoped by default. It enumerates paths and minimum identity, lifecycle, dependency, and cycle metadata before loading content; then it reads only the exact target, its dependency closure, and applicable archived summaries. Lifecycle scans remain metadata-only. Exact cycle retrieval bounds archive work, while broad bodies require explicit audit intent. Deterministic validation may scan the complete projection but returns compact diagnostics rather than injecting the corpus into agent context.
